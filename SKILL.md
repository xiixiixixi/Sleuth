...
在开始任何操作前，先判断问题类型。如果 sleuth-output/ 中有相关历史文件，优先评估能否复用。
...
```bash
find "${CLAUDE_SKILL_DIR}/../sleuth-output/" ~/.sleuth/output/ ~/.sleuth/sessions/ -type f \( -name "*.json" -o -name "*.txt" -o -name "*.md" -o -name "*.png" \) 2>/dev/null | head -30
```

或定向搜索相关关键词：

```bash
grep -rl "关键词" "${CLAUDE_SKILL_DIR}/../sleuth-output/" ~/.sleuth/output/ ~/.sleuth/sessions/ 2>/dev/null | head -20
```
...
| 持久化 | 不需要保存文件 | 可能需要保存截图/PDF/数据 |

**简单问题** → 走快速通道（见[前置检查 > 问题分类](#问题分类)）。如果已知答案直接回复；如果不确定，用一个搜索 + 一个页面验证后立即回复。跳过完整前置检查（不运行 check-deps，不创建会话日志，不写交付文件），读完即关 tab。

**复杂问题** → 走完整流程：前置检查 → 多角度搜索 → 子 Agent 循环调研 → 交叉验证 → 结果交付。

如果不确定属于哪类，先按简单问题处理——打开一个页面看了一下信息不够，再升级为完整流程。**升级时**：先关闭简单路径打开的 tab，然后从「环境检查与初始化」开始完整流程（运行 check-deps、创建会话日志等）。
...
Agent({
  ...
  prompt: "在 sleuth-output/ 和 ~/.sleuth/sessions/ 中搜索关于「关键词」的相关文件。返回：是否找到、文件路径、修改时间、时效性评估。不要返回文件内容。"
})
```
...
1. **结果交付**：文本内联总结，文件持久化到 `sleuth-output/` 对应子目录。汇报时列出所有交付文件路径和简介
2. **关闭 tab**：子 Agent 应自行关闭其创建的 tab。Stop hook 会兜底关闭残留 tab，但建议手动确认
3. **临时文件清理**：仅删 `/tmp` 下中间产物；sleuth-output/ 不清除
4. **结束会话日志**：手动执行 `node "${CLAUDE_SKILL_DIR}/scripts/session-logger.mjs" --action finish --sid $SID --outcome success|partial|fail`。Stop hook 会自动 finish 遗漏的 session（标记 partial），但手动 finish 更及时准确
5. **更新站点经验**：发现新的有效模式或陷阱时写入 `references/site-patterns/<域名>.md`，然后运行 `node "${CLAUDE_SKILL_DIR}/scripts/update-site-stats.mjs" --domain <域名>`。Stop hook 会为复杂站点（遇到 CAPTCHA/登录墙/付费墙，或累计访问 3+ 次的域名）自动创建 stub 并刷新统计；定性经验仍需手动写入
...
→ 找到答案后在聊天中直接回复，附来源 URL。不创建会话日志、不写文件、读完即关 tab。
...
**保留策略**：sleuth-output/ 下的文件保留 **7 天**。check-deps.mjs 每次运行自动清理超过 7 天的过期日期目录（含其下所有 session 目录）。也可手动运行：

```bash
node "${CLAUDE_SKILL_DIR}/scripts/cleanup-output.mjs" [--days N] [--dry-run]
```

**已有输出复用**：处理任何请求前，先按[缓存优先判定](#缓存优先判定所有问题适用)的规则评估能否复用已有文件——查文件修改时间、判断时效性级别、按分级策略决定直接用/标注后复用/弃用重搜。

**按 session 查找产出**：如需定位特定 session 的交付文件，可从 session 日志的 `operations` 中获取文件路径：

```bash
# 查看某 session 的所有交付记录
node -e "const s=require('$HOME/.sleuth/sessions/<sid>.json'); s.operations.filter(o=>o.type==='deliver').forEach(o=>console.log(o.file))"
# 或直接列出 session 输出目录
...
特定网站经验按域名存储在 `references/site-patterns/` 下。确定目标域名后，通过 `match-site.mjs` 查找匹配的站点经验文件——这是查询站点模式的主要方式。读取匹配文件获取先验知识（平台特征、有效模式、已知陷阱）。经验标注发现日期，当作可能有效的提示而非保证——按经验失败时回退通用模式。
...
| `references/site-patterns/{domain}.md` | 确定目标网站后通过 match-site.mjs 匹配并读取 |
