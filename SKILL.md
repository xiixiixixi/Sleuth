...
所有搜索 Agent 的非浏览器工作继续并行；主 Agent 可以把同次 full 检查得到的 `SLEUTH_CDP_PORT` 和 `SLEUTH_CDP_WS` 同时注入多个任务。只有单条浏览器命令通过 `shared-browser.mjs exec` 短暂排队：协调器自动选择该 Agent 自己的唯一标签，执行命令后再次选回该标签，再核对最终 URL 和标题并自动释放短锁。禁止手动 `acquire/release`，禁止跨命令复用 `@eN`，禁止 `--new-tab` / `window.open` 新建无归属标签，也禁止绕过协调器或用 `--session` / `--namespace` 制造额外连接。
...
- 浏览器兜底只连接现有登录态 Chrome；禁止新开无登录态浏览器、下载测试浏览器、关闭用户已有标签页或用固定等待掩盖工具失败。
