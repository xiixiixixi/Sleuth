...
5. **更新站点经验**：发现新的有效模式或陷阱时写入 `~/.sleuth/site-patterns/<域名>.md`，然后运行 `node "${CLAUDE_SKILL_DIR}/scripts/update-site-stats.mjs" --domain <域名>`。Stop hook 会为复杂站点（遇到 CAPTCHA/登录墙/付费墙，或累计访问 3+ 次的域名）自动创建 stub 并刷新统计；定性经验仍需手动写入
