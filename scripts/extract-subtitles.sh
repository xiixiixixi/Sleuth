...
#   2. yt-dlp 提取人工英文字幕
#   3. yt-dlp 提取自动生成字幕（中文优先）
#   4. 检查页面 HTML 中是否有 transcript 链接或全文
#   5. 所有策略失败 → 输出诊断信息和替代方案
#
# 依赖：yt-dlp、curl
#
# 注意：本脚本不调用任何付费 API。对无字幕的纯音频内容，
#       会给出用户可操作的替代方案（如本地 Whisper 转录）。
...
# 检查页面本身是否包含大段文字（可能是全文 transcript）
