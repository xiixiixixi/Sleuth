...
```bash
find ~/.sleuth/output/ ~/.sleuth/sessions/ -type f \( -name "*.json" -o -name "*.txt" -o -name "*.md" -o -name "*.png" \) 2>/dev/null | head -30
```

或定向搜索相关关键词：

```bash
grep -rl "关键词" ~/.sleuth/output/ ~/.sleuth/sessions/ 2>/dev/null | head -20
```

当文件较多时，用 Explore subagent 搜索缓存，避免大量文件路径占满主 context：
...
**硬性规则：只有主 Agent 能调用 Agent 工具派子 Agent。子 Agent 是纯执行者，禁止嵌套派发。**
...
**核心原则**：
- **子 Agent 是叶子执行者，禁止使用 Agent 工具**。子 Agent 不能再派子 Agent，所有搜索自己用 agent-browser 直接完成。
- 子 Agent prompt 中**禁止写「加载 sleuth skill」**，改为「先 Read references/subagent-guide.md，严格遵循其中的指引」
