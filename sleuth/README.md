...
|------|------|------|
| **直答** | 已有知识足够、无需联网 | 直接回复 |
| **快速验证** | 一次搜索可确认 | 无 session，搜索后内联回复 |
| **定向搜索** | 多步搜索、单一主题 | 开 session，deliver 交付 |
| **深度调研** | 多源交叉验证、并行探针 | Frame → Search → Review → Deliver |

每级都先执行 Recall（历史召回），有命中则跳过重复搜索。
...
深度调研中，主 Agent 担任总指挥，拆解搜索角度后分发探针（subagent）。每个探针：
- 通过 `run_in_background: true` 异步执行
- 浏览器通过 `--session <name>` 隔离 tab
- 遵循统一搜索方法论（`references/search-guide.md`）
- 完成后 `deliver save` 交付发现

主 Agent 收集所有探针结果后，交叉验证 → 合并 → 输出最终结论。
