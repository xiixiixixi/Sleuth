...
**工具名说明**：以下用能力描述指代工具——使用你运行时对应的工具。括号内为 Claude Code 工具名作为参考。

| 能力 | 适合做什么 | 证据边界 | Claude Code |
|------|------------|----------|-------------|
| **网络搜索** | 发现候选、别名、关键词地图 | snippet 有偏置，不能直接当证明 | `WebSearch` |
| **网页读取** | 快速读正文、扫静态页面 | 不保证动态内容、登录态、页面真实状态 | `WebFetch` |
| **浏览器操控** | 一手验证、动态渲染、登录、交互 | 适合高价值验证，不适合大范围扫网页 | `agent-browser` |
