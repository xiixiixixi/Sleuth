...
- **缺正文**（知道在哪但没读内容）→ WebFetch / reader 先拿；拿不到或不满意 → 浏览器
- **缺证据强度**（需要确认真实性）→ reader 结果只是线索；核心结论必须回原始来源验证；不确定 reader 给的是不是真的 → 浏览器
- **缺交互 / 登录态 / 动态内容** → 浏览器
...
- 是否需换路：当前路径是不是只剩 SEO 文 / 营销页 / reader 失真
...
- **呈现型**（报告本身需要给人看：产品图 / 对比图表 / 官方规格图 / UI / 示意图）：**归档 + 内嵌**
  1. 下载到本地（命令看 `${CLAUDE_SKILL_DIR}/references/tool-guide.md`）
  2. 报告里用**来源 URL** 内嵌 `![图注](来源URL)`（可移植，GitHub / 各 viewer 都能渲染）
