...
| 工具 | 用途 | 边界 |
|------|------|------|
| **WebSearch** | 发现候选实体、概念、对比框架 | snippet 是线索，不是结论 |
| **WebFetch / reader** | 读搜索结果页面正文，提取实体名和 URL | 不做深度验证 |

不用 agent-browser——侦察阶段不需要动态渲染或登录态。
