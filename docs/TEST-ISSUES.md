...
> 更新：2026-08-11。每个问题只保留一个当前状态；“代码已改”和“真实环境已验证”分开写。

## 当前结论

核心数据链、跨 Agent 线索、检查门、视觉证据和审查交付已经通过自动化测试和真实任务。轻量工具失败后的浏览器交接、CLI 自动安装、Node 版本边界和“只连接现有登录态 Chrome”已有行为测试。当前 9222 端口已经核验为用户日常使用的稳定版 Google Chrome；浏览器兜底不再接受 `~/.sleuth/chrome-live`、Chrome for Testing、Chrome for Dev、Chromium 或其他独立用户目录。搜索 Agent 保持并行，浏览器命令统一通过动作级短锁复用一个默认后台连接；真实双标签并发验证中 A/B 两次都读回自己的页面，失败自动释放锁，测试标签已关闭。
...
| 030 | 多个搜索角色并发使用同一现有 Chrome 时，“当前标签页”会串页；`--session` 不能隔离同一 CDP 连接的活动标签 | 已改为动作级短锁并完成真实双标签验证 | 所有搜索 Agent 继续并行，只有单条浏览器命令短暂排队；`shared-browser.mjs` 在同一批次于命令前后都选择 owner 唯一标签，再核对 URL/标题。真实并发中 A 始终为 example.com、B 始终为 example.org，测试标签均已关闭 |
...
| 033 | 每条浏览器命令可能新建一个长期驻留的控制进程，其他常驻 CDP 代理也可能持续占用 9222，导致 Chrome 反复弹出远程调试授权框 | `agent-browser` 部分已修复并完成真实复验 | 当前只复用默认后台服务并显式传 `--idle-timeout 0`；本轮系统只有一个 `agent-browser` 后台进程、一个 `default.sock` 和一条由它建立的 9222 连接。另有其他工具既有的 Node CDP 连接，按用户确认范围不由本协调器管理，也未擅自停止 |
...
## 2026-08-11 共享连接与并行浏览器操作复验

| # | 问题 | 当前状态 | 修复证据 |
|---|---|---|---|
| 035 | 任务级 `acquire/run/release` 会让搜索 Agent 一个等一个，且不同项目的同名 Agent 可能误用同一标签 | 已取消并完成自动及真实验证 | 子 Agent 只有 `shared-browser.mjs exec` 单一入口；锁只覆盖一次浏览器命令，成功或失败都自动释放。标签身份由任务目录、轮次和 Agent 名共同生成；多进程假 CLI 测试证明两个批次不重叠、标签不交叉，真实 Chrome 并发导航和再次读取均没有串页 |
| 036 | 首版动作锁在异常进程回收、标签选择失败、新标签切换和环境变量继承上仍可能造成抢页或误页 | 已修复并完成自动及真实验证 | 强制终止留下的锁禁止自动强拆；标签必须先经结构化列表确认唯一，选择失败不创建；命令后再次选回 owner，禁止无归属新标签；生产锁目录固定，危险环境变量被清除，底层 CLI 使用协调器固定空配置。独立审查提出的异常路径均已加入回归测试 |

本次按 `TESTING.md` 复验结果：

1. `node --test scripts/__tests__/*.mjs`：151 passed，0 failed；新增测试覆盖动作级互斥、A/B 标签边界、正常失败即时释放、异常残留锁拒绝强拆、选择失败与重名标签阻断、危险参数和环境变量拒绝、不同任务同名 Agent 的身份隔离及搜索 prompt 的并行契约。
2. `node scripts/check-docs.mjs`：29 个 Markdown 文件通过；全部 Node/Bash 语法和 `git diff --check` 通过。
3. full 检查为 `ready:true`、`browser_identity: verified-user-chrome`、Node.js 24.16.0、`agent-browser` 0.33.2，目标是用户日常稳定版 Google Chrome，端口 9222。
4. 两个 wrapper 进程同时发起：A 打开 `https://example.com/`，B 打开 `https://example.org/`；各自同批次的 URL/标题核验正确。再次同时执行 `get url`，A/B 仍分别返回自己的页面。
5. 每次命令结果均显示 `reused:true`、`restartedBackground:false`；结束后 `status --json` 为 `locked:false`、`stale:false`。系统只有一个 `agent-browser` 后台进程、一个 `default.sock` 和一条由它建立的 9222 连接。
6. 9222 上另有一个本次修改前已经存在的普通 Node CDP 连接；它不属于 `agent-browser`，按本次范围只记录、不停止。
7. 加固后的最终复验只关闭了 `sleuth-hardened-a-0811` 和 `sleuth-hardened-b-0811` 两个测试标签；两次关闭均返回 `closed:true`，用户原有标签和 Chrome 未关闭。

## 2026-08-09 身份结论修正与完整复验
